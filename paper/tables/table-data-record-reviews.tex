...
\caption{Data record overview for review records in \webisSRCorpus. \textit{Key(s)} denotes the top-level JSON keys in the released file, and \textit{Description} summarizes their content.}
\label{tab:data-record-reviews}

\begin{tabular}{@{}p{1.9cm}p{4.8cm}p{7.7cm}@{}}
\toprule
\textbf{\textsf{Group}} & \textbf{\textsf{Key(s)}} & \textbf{\textsf{Description}} \\
\midrule

Identifiers &
\texttt{id}, \texttt{doi} &
OpenAlex work identifier and DOI for the review record. \\

Metadata &
\texttt{title}, \texttt{abstract}, \texttt{year}, \texttt{type}, \texttt{source}, \texttt{language} &
Core bibliographic metadata for each review. \\

Subject labels &
\texttt{field}, \texttt{subfield}, \texttt{topics}, \texttt{keywords} &
OpenAlex field assignments and topical annotations. \\

Authorship &
\texttt{authors} &
Author identifiers, names, and affiliation strings, where available. \\

Citation data &
\parbox[t]{4.8cm}{\texttt{cited\_by\_count}, \texttt{referenced\_works\_count}, \texttt{referenced\_works}, \texttt{references\_abstract\_}\\\texttt{coverage}} &
Citation counts, resolved OpenAlex reference list, and summary information about abstract coverage among referenced works. \\

Access &
\texttt{pdf\_url} &
Source PDF link when OpenAlex provides one. \\

Review aims &
\texttt{objective}, \texttt{research\_questions} &
Extracted statements of the review objective and explicit research questions. \\

Search strategy &
\parbox[t]{4.8cm}{\texttt{keywords\_used}, \texttt{exact\_boolean\_queries}, \texttt{databases\_used}, \texttt{snowballing}} &
Extracted search terms, normalized Boolean queries, named databases, and citation-chasing indicator. \\

Eligibility and flow &
\parbox[t]{4.8cm}{\texttt{inclusion\_criteria}, \texttt{exclusion\_criteria}, \texttt{n\_studies\_initial}, \texttt{n\_studies\_final}} &
Extracted eligibility criteria and study-count information from the review workflow. \\

Temporal scope &
\parbox[t]{4.8cm}{\texttt{year\_range}, \texttt{year\_range\_normalized}, \\\texttt{year\_range\_normalization\_rule}} &
Reported search-year restrictions, their normalized form, and the applied normalization rule. \\
