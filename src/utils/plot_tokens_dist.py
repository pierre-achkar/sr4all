...
def main():
    ...
    print("-" * 40)
    print(
        f"Statistical Outliers: {len(extreme_outliers)} docs ({len(extreme_outliers)/len(tokens):.1%} of corpus)"
    )
    print(
        f"Original Limit (>28k): {len(tokens[tokens > Config.CONTEXT_LIMIT_1])} docs ({len(tokens[tokens > Config.CONTEXT_LIMIT_1])/len(tokens):.2%} of corpus)"
    )
    print(
        f"CRITICAL (>121k):     {len(over_limit)} docs ({len(over_limit)/len(tokens):.2%} of corpus)"
    )
    print("-" * 40)
    ...
    ax1.boxplot(
        tokens, vert=True, patch_artist=True, boxprops=dict(facecolor="lightblue")
    )
    ...
    ax2.boxplot(
        tokens, vert=True, patch_artist=True, boxprops=dict(facecolor="lightgreen")
    )
    ax2.set_yscale("log")
    ...
    ax2.axhline(
        y=Config.CONTEXT_LIMIT, color="orange", linestyle="--", label="128k Context"
    )
    ax2.legend()
