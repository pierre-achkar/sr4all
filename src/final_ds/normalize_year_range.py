...
CONFIG = {
    "input_jsonl": Path("/home/fhg/pie65738/projects/sr4all/data/final/sr4all_full.jsonl"),
    "output_jsonl": Path("/home/fhg/pie65738/projects/sr4all/data/final/sr4all_full_normalized_year_range.jsonl"),
    "log_file": Path("/home/fhg/pie65738/projects/sr4all/logs/final_ds/normalize_year_range.log"),
...
_PRESENT_RE = re.compile(r"\b(present|current|to date|to-date|now|today)\b", re.IGNORECASE)
_LAST_RE = re.compile(r"\b(last|past|previous|recent)\s+(\d{1,3})\s+years?\b", re.IGNORECASE)
_SINCE_RE = re.compile(r"\b(since|from)\b", re.IGNORECASE)
_DECADE_RE = re.compile(r"\b(last|past|previous|recent)\s+(\d{1,2})\s+decades?\b", re.IGNORECASE)
...
def normalize_year_range(raw: Optional[str], study_year: Optional[int]) -> Tuple[Optional[str], Optional[str]]:
    if not raw:
        if study_year:
    ...
    if len(years) >= 2:
        ...
        candidate = f"{y1}-{y2}"
        return (candidate, "two_years") if _is_valid_range(candidate) else (None, "invalid_two_years")

    # Case 2: year to present/current
    if len(years) == 1 and _PRESENT_RE.search(text):
        if study_year:
            candidate = f"{years[0]}-{study_year}"
            return (candidate, "year_to_present") if _is_valid_range(candidate) else (None, "invalid_year_to_present")
    ...
    if len(years) == 1 and _SINCE_RE.search(text):
        if study_year:
            candidate = f"{years[0]}-{study_year}"
            return (candidate, "since_year") if _is_valid_range(candidate) else (None, "invalid_since_year")
    ...
    if m and study_year:
        ...
        return (candidate, "last_n_years") if _is_valid_range(candidate) else (None, "invalid_last_n_years")
    ...
    if m and study_year:
        ...
        if n:
            start = max(study_year - n + 1, 0)
            candidate = f"{start}-{study_year}"
            return (candidate, "last_word_years") if _is_valid_range(candidate) else (None, "invalid_last_word_years")
    ...
    if m and study_year:
        ...
        candidate = f"{start}-{study_year}"
        return (candidate, "last_n_decades") if _is_valid_range(candidate) else (None, "invalid_last_n_decades")

    # Case 7: last/past decade (single)
    if _DECADE_WORD_RE.search(text) and study_year:
        start = max(study_year - 10 + 1, 0)
        candidate = f"{start}-{study_year}"
        return (candidate, "last_decade") if _is_valid_range(candidate) else (None, "invalid_last_decade")

    # Case 8: last/past few years
    if _FEW_RE.search(text) and study_year:
        start = max(study_year - 5 + 1, 0)
        candidate = f"{start}-{study_year}"
        return (candidate, "last_few_years") if _is_valid_range(candidate) else (None, "invalid_last_few_years")
