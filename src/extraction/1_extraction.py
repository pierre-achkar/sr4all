...
def main():
    ...
    logger.info(f"Starting extraction on {total_remaining} docs in batches of {batch_size}...")
    ...
    for i in tqdm(range(0, total_remaining, batch_size), desc="Processing Batches"):
        ...
        for record in batch_records:
            ...
            try:
                ...
                if not path.exists():
                    error_results.append(_create_error(doc_id, file_path_str, "FILE_NOT_FOUND"))
                    continue
                
                text = path.read_text(encoding="utf-8", errors="replace")
                if not text.strip():
                    error_results.append(_create_error(doc_id, file_path_str, "EMPTY_TEXT"))
                    continue
                ...
                error_results.append(_create_error(doc_id, file_path_str, f"READ_ERROR: {e}"))
