...
def _fix_lucene_syntax(text: Optional[str]) -> Tuple[str, bool]:
    ...
    def flush():
        ...
        if len(buffer) > 1:
            ...
            if term: processed_tokens.append({"type": "TERM", "val": term})
        buffer.clear()
    ...
    for t in processed_tokens:
        if t["type"] == "OPERATOR":
            if not cleaned_tokens or (cleaned_tokens[-1]["val"] == "("): continue
    ...
    if open_c > close_c: res += ")" * (open_c - close_c)
    elif close_c > open_c:
        for _ in range(close_c - open_c): res = res.replace(")", "", 1)
        
    final_query = _strip_onion_layers(res.strip())
    return final_query, final_query != original

def is_valid(text: str) -> bool:
    if not text or re.match(r"^[\s()]*$", text): return False
    tokens = _tokenize_query(text)
...
def main():
    ...
    with input_path.open("r", encoding="utf-8") as fin, temp_path.open("w", encoding="utf-8") as fout:
        for line in fin:
            ...
            meaningful = [q for q in queries if isinstance(q, str) and _is_meaningful_query(q)]
            ...
            if not has_any_operator:
                ...
                for mq in meaningful:
                    ...
                    if is_valid(fixed_q):
                        ...
                        if repaired: m["q_rep"] += 1
                    else:
                        m["q_drop"] += 1
                
                rec["boolean_queries"] = fixed_list if fixed_list else None
                if not fixed_list: m["nulled"] += 1

            fout.write(json.dumps(rec, ensure_ascii=False) + "\n")

    temp_path.replace(output_path)
    logger.info(f"Done. Records: {m['records']} | Nulled: {m['nulled']} | Repaired Queries: {m['q_rep']} | Dropped: {m['q_drop']}")
